...
fun CategoryRecipePreviewEntity.toDto(): RecipePreviewDto =
    RecipePreviewDto(
        recipeId = null,
        id = id,
        name = name,
...
fun RecipePreviewDto.toCategoryEntity(categoryOverride: String? = null): CategoryRecipePreviewEntity =
    CategoryRecipePreviewEntity(
        id =
            id?.takeIf { it.isNotBlank() }
                ?: recipeId?.takeIf { it.isNotBlank() }
                ?: throw IllegalStateException("Both 'id' and 'recipe_id' are null or blank"),
